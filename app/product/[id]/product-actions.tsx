...
export default function ProductActions({ product }: ProductActionsProps) {
  ...
  return <div className="product-actions"><button className="button button-dark product-add-button" disabled={added} onClick={addToCart}>{added ? "Додадено ✓" : "Додај во кошничка"}</button>{added && <p className="product-added-message">Парчето е додадено во твојата кошничка.</p>}</div>;
}
